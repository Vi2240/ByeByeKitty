...
public class EnemyHealth : MonoBehaviour
{
    ...
    LvlAndXpVault lvlAndXpVault;

    void Start()
    {
        currentHealth = maxHealth;
        healthBar.SetHealth(currentHealth, maxHealth);
        lvlAndXpVault = FindAnyObjectByType<LvlAndXpVault>();
    }
    ...
    void Die()
    {
        // Add XP to player on enemy death
        if (lvlAndXpVault != null)
        {
            float xpReward = 5f; // Or calculate based on enemy type/level
            lvlAndXpVault.AddPlayerXp(xpReward);
        }
